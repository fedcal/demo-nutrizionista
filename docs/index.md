...
hero:
  name: "Demo Nutrizionista"
  text: "Template Angular 21 SSR"
  tagline: "Sito vetrina pre-configurato con dati mock realistici per PMI italiane"
  actions:
    - theme: brand
      text: Demo live
      link: https://nutrizionista.demo.federicocalo.dev
    - theme: alt
      text: Source code
      link: https://github.com/fedcal/federico-demo-nutrizionista
...
features:
  ...
  - icon: 📊
    ...
    details: Dati Italia 2026 (menu, listini, orari, prezzi medi) curati per il verticale Nutrizionista.
  - icon: 🚀
...
Questo repository contiene il template **Angular 21 SSR** per un sito web professionale per **Nutrizionista** in Italia.
...
Il mercato italiano delle PMI per **Nutrizionista** ha caratteristiche specifiche:
...
```bash
git clone https://github.com/fedcal/federico-demo-nutrizionista.git
cd federico-demo-nutrizionista
npm install
npm start
# → http://localhost:4200
```
...
```bash
npm run build
npm run serve:ssr:nutrizionista
# → http://localhost:4000
```
...
- [GitHub repo](https://github.com/fedcal/federico-demo-nutrizionista)
